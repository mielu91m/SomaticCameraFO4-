...
namespace Systems {
	...
	HRESULT STDMETHODCALLTYPE Graphics::Hook_Present(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
	{
		...
		if (!pluginGraphics->m_Initialized)
		{
			if (SUCCEEDED(pSwapChain->GetDevice(IID_PPV_ARGS(&pluginGraphics->m_Device))))
			{
				...
				pluginGraphics->m_UI.reset(UI::CreateMenu(pluginGraphics->m_OutputWindow));
	...
	struct DXGIPresentHook {
		...
		static void thunk(std::uint32_t a_timer)
		{
			...
			if (!pluginGraphics->m_Initialized)
			{
				// FO4: Internal DXGI hook not yet implemented
				// Use overlay mode instead
				return;
			}

			if (pluginGraphics->m_UI)
			{
				pluginGraphics->m_UI->BeginFrame();
				pluginGraphics->m_UI->OnUpdate();
				pluginGraphics->m_UI->EndFrame();
		...
		if (getD3D11)
		{
			...
			else
			{
				...
				if (m_Window->m_Initialized)
				{
					...
					// FO4: Internal DXGI present hook needs address library ID
					// m_DXGIPresentAddress = REL::RelocationID(XXXXX).address() + 0x9;
	...
	{
		auto pluginFO4 = DLLMain::Plugin::Get()->Fallout4();

		std::string fullFilePath = pluginFO4->Path() + fileName;
	...
	DWORD WINAPI Graphics::CreateDevice(LPVOID)
	{
		...
		if (!pluginGraphics->HookOverlay())
		{
			...
			HRESULT hr = D3D11SystemCreateDeviceAndSwapChain(
				nullptr,
